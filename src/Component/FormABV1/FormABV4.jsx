    ...
    const handleSubmit = async (e) => {
        ...
        for (let i = 0; i < participants.length; i++) {
            ...
            if (p.CedulaParticipante.length < 8) {
                alert(`La Cédula del participante #${num} debe tener al menos 8 dígitos.`);
                setIsSubmitting(false);
                return;
            }
        ...
        try {
            ...
            const validityFormula = `OR(${validatorIds.map(id => `{IDValidadorTardeFecha1} = '${id}'`).join(',')})`;
            const validRecords = await validationTable.select({ filterByFormula: validityFormula, fields: ['IDValidadorTardeFecha1'] }).all();
            
            if (validRecords.length !== validatorIds.length) {
                const found = new Set(validRecords.map(rec => rec.get('IDValidadorTardeFecha1')));
        ...
        <div className="container">
            <div className="form-wrapper">
                <div className="header-card">
                    ...
                    <h2>4 de noviembre de 2025 - 8:00 p.m. a 12:00 p.m. y 5 de noviembre de 2025 1:00 p.m. a 5:00 p.m. TURNO: TARDE</h2>
                </div>
